...
// persistent circular K/V cache.
//
// The generic path uses one block per (head, query, batch). Scores, optional
// relative-position terms, softmax, and the attn*V context are computed in one
// kernel, so intermediate score matrices are not written to global memory.
//
...
static __device__ __forceinline__ float attention_warp_sum(float value) {
#pragma unroll
...
// Unlike attention_warp_sum(), return the complete sum to every lane. The
// register-resident Q=2 kernel needs each lane to retain the softmax weight
// for the four V features it accumulates.
static __device__ __forceinline__ float attention_warp_all_sum(float value) {
#pragma unroll
    for (int offset = 16; offset > 0; offset >>= 1) {
...
static __device__ __forceinline__ float4 attention_load4(const float * ptr) {
    return *reinterpret_cast<const float4 *>(ptr);
}

static __device__ __forceinline__ float4 attention_load4(const half * ptr) {
...
static __device__ __forceinline__ float4 attention_load_kv4(
    ...
    if constexpr (Cached) {
        if (j < cache_len) {
            ...
            return attention_load4(cache_head + (size_t) physical_j * cache_sj + d4);
        }
        return attention_load4(chunk_head + (size_t) (j - cache_len) * chunk_sj + d4);
    }
    return attention_load4(chunk_head + (size_t) j * chunk_sj + d4);
}

template <typename T, bool Cached>
static __device__ __forceinline__ float attention_load_kv(
        const T * chunk_head, const float * cache_head, int j, int cache_len,
...
static __global__ void fused_cached_attention_q1_d64_kernel(
        ...
        const float * __restrict__ Vcache, const int32_t * __restrict__ slot_ids,
        const int32_t * __restrict__ ring_heads,
        const int32_t * __restrict__ active_lengths,
    ...
    const float * Qh = Q + (size_t) b * q_sb + (size_t) h * q_sh;
    ...
    for (int j = key_begin + warp; j < kv; j += warps) {
        const float2 k2 = attention_load_kv2<T, Cached>(
...
#pragma unroll
    ...
    for (int j = key_begin + part; j < kv; j += value_parts) {
        context += scores[j] * attention_load_kv<T, Cached>(
...
// One-block-per-query specialization for d_k=128 and q=2. Score-producing
// warps retain reduction state to reduce synchronization and scratch use.
template <typename T, bool Cached>
static __global__ void fused_relpos_attn_warp_128_kernel(
    ...
    for (int j = warp; j < kv; j += RELPOS_ATTN_WARPS_128) {
        const float4 k4 = attention_load_kv4<T, Cached>(
            Kh, Kch, j, cache_len, ring_head, k_sj, cache_sj, d4);
        const int row = 1 + j - i;
        const float4 p4 = attention_load4(Ph + (size_t) row * p_sr + d4);
        float score =
...
#pragma unroll
    ...
    for (int j = 0; j < kv; ++j) {
        context +=
            sc[j] * attention_load_kv<T, Cached>(
                        Vh, Vch, j, cache_len, ring_head, v_sj, cache_sj, d);
    }
...
static __global__ void fused_relpos_attn_q2_warp_128_kernel(
    ...
    for (int j = warp; j < kv; j += RELPOS_ATTN_WARPS_128) {
        const float4 k4 = attention_load_kv4<T, Cached>(
            Kh, Kch, j, cache_len, ring_head, k_sj, cache_sj, d4);
        const float4 p04 = attention_load4(Ph + (size_t) (j + 1) * p_sr + d4);
        const float4 p14 = attention_load4(Ph + (size_t) j * p_sr + d4);
        float score0 =
        ...
        score0 = attention_warp_sum(score0);
        score1 = attention_warp_sum(score1);
        if (lane == 0) {
...
#pragma unroll
    ...
    local_sum0 = attention_warp_sum(local_sum0);
    local_sum1 = attention_warp_sum(local_sum1);
    if (lane == 0) {
...
// Register-resident Q=2, KV=72 specialization. Each warp owns consecutive
// cache positions and reuses K/V across both query rows.
template <typename T>
__global__ __launch_bounds__(256, 4) void fused_relpos_attn_q2_register_kernel(
    ...
    const float4 q04 = attention_load4(Qh0 + d4);
    const float4 q14 = attention_load4(Qh1 + d4);
    const float4 bu4 = attention_load4(bu + (size_t) h * RELPOS_ATTN_DK_128 + d4);
    const float4 bv4 = attention_load4(bv + (size_t) h * RELPOS_ATTN_DK_128 + d4);
    ...
    float4 p14 = attention_load4(Ph + (size_t) j0 * p_sr + d4);
#pragma unroll
    for (int c = 0; c < keys_per_warp; ++c) {
        ...
        if (c < 7 || j < cache_len) {
            ...
            k4 = attention_load4(Kch + (size_t) physical_j * cache_sj + d4);
        } else {
            k4 = attention_load4(Kh + (size_t) (j - cache_len) * k_sj + d4);
        }
        float score0 =
        ...
        score0 = attention_warp_all_sum(score0);
        score1 = attention_warp_all_sum(score1);
        const float additive_mask = Mb ? Mb[j] : 0.0f;
...
#pragma unroll
    for (int c = 0; c < keys_per_warp; ++c) {
        ...
        if (c < 7 || j < cache_len) {
            ...
            v4 = attention_load4(Vch + (size_t) physical_j * cache_sj + d4);
        } else {
            v4 = attention_load4(Vh + (size_t) (j - cache_len) * v_sj + d4);
        }
...
// Register-resident Q=4, cache=56, KV=60 specialization. Each warp reuses K/V
// across all query rows.
template <typename T>
__global__ __launch_bounds__(192, 3) void fused_relpos_attn_q4_register_kernel(
    ...
    const float4 q04 = attention_load4(Qh0 + d4);
    const float4 q14 = attention_load4(Qh1 + d4);
    const float4 q24 = attention_load4(Qh2 + d4);
    const float4 q34 = attention_load4(Qh3 + d4);
    const float4 bu4 = attention_load4(bu + (size_t) h * RELPOS_ATTN_DK_128 + d4);
    const float4 bv4 = attention_load4(bv + (size_t) h * RELPOS_ATTN_DK_128 + d4);
...
#pragma unroll
    for (int c = 0; c < keys_per_warp; ++c) {
        const int j = j0 + c;
        const float4 k4 = attention_load_kv4<T, true>(
            Kh, Kch, j, cache_len, ring_head, k_sj, cache_sj, d4);
        const float4 p04 = attention_load4(Ph + (size_t) (j + 3) * p_sr + d4);
        const float4 p14 = attention_load4(Ph + (size_t) (j + 2) * p_sr + d4);
        const float4 p24 = attention_load4(Ph + (size_t) (j + 1) * p_sr + d4);
        const float4 p34 = attention_load4(Ph + (size_t) j * p_sr + d4);
#define RELPOS_SCORE(k, qc, p, qp) \
        ((k).x * (qc).x + (p).x * (qp).x + (k).y * (qc).y + (p).y * (qp).y + \
         (k).z * (qc).z + (p).z * (qp).z + (k).w * (qc).w + (p).w * (qp).w)
        float score0 = attention_warp_all_sum(RELPOS_SCORE(k4, qc0, p04, qp0));
        float score1 = attention_warp_all_sum(RELPOS_SCORE(k4, qc1, p14, qp1));
        float score2 = attention_warp_all_sum(RELPOS_SCORE(k4, qc2, p24, qp2));
        float score3 = attention_warp_all_sum(RELPOS_SCORE(k4, qc3, p34, qp3));
#undef RELPOS_SCORE
        const float additive_mask = Mb ? Mb[j] : 0.0f;
        score0 = score0 * scale + additive_mask;
        score1 = score1 * scale + additive_mask;
...
#pragma unroll
    for (int c = 0; c < keys_per_warp; ++c) {
        const int j = j0 + c;
        const float4 v4 = attention_load_kv4<T, true>(
...
// Paired-query path for common cache-aware geometries. Pairing reduces the
// block count and reuses K/V across query rows.
template <typename T>
...
#pragma unroll
    for (int c = 0; c < keys_per_warp; ++c) {
        ...
        if (j < kv_len) {
            ...
            const float4 p04 = attention_load4(Ph + (size_t) pos0 * p_sr + d4);
#define RELPOS_SCORE(k, qc, p, qp) \
            ((k).x * (qc).x + (p).x * (qp).x + (k).y * (qc).y + (p).y * (qp).y + \
             (k).z * (qc).z + (p).z * (qp).z + (k).w * (qc).w + (p).w * (qp).w)
            score0 = attention_warp_all_sum(RELPOS_SCORE(k4, qc0, p04, qp0));
            if (has_query1) {
                const float4 p14 = attention_load4(Ph + (size_t) (pos0 - 1) * p_sr + d4);
                score1 = attention_warp_all_sum(RELPOS_SCORE(k4, qc1, p14, qp1));
            }
#undef RELPOS_SCORE
            const float additive_mask = Mb ? Mb[j] : 0.0f;
            score0 = score0 * scale + additive_mask;
...
static int relpos_attn_warp_128_max_blocks_per_sm(int device, size_t shmem) {
    // Dynamic shared memory is fixed for a given kernel shape, so cache the
    // occupancy query per device.
...
template <typename T, bool Cached>
static __global__ void fused_attention_kernel(
        const float * __restrict__ Q, const T * __restrict__ K,
        const T * __restrict__ V, const T * __restrict__ Ppos,
        const float * __restrict__ bu, const float * __restrict__ bv,
        const float * __restrict__ mask, float * __restrict__ ctx,
        ...
        const int32_t * __restrict__ ring_heads,
        const int32_t * __restrict__ active_lengths,
        int q, int kv, int n_head, int cache_len, float scale, bool relative,
    ...
    const float * Vch = Cached
        ...
        : nullptr;
    const T *     Ph  = relative ? Ppos + (size_t) h * p_sh : nullptr;
    const float * Mb  = mask ? mask + (size_t) b * m_sb + (size_t) i * m_sq : nullptr;

    Qu[d] = Qhi[d] + (relative ? bu[h * dk + d] : 0.0f);
    Qv[d] = Qhi[d] + (relative ? bv[h * dk + d] : 0.0f);
    __syncthreads();
    ...
    if (dk == 128) {
        ...
        for (int j = key_begin + warp; j < kv; j += nw) {
            const int row = (q - 1) + j - i;  // rel-shift index
            const float4 k4 =
                attention_load_kv4<T, Cached>(
                    Kh, Kch, j, cache_len, ring_head, k_sj, cache_sj, lane * 4);
            float s =
                k4.x * Qu[lane * 4 + 0] + k4.y * Qu[lane * 4 + 1] +
                k4.z * Qu[lane * 4 + 2] + k4.w * Qu[lane * 4 + 3];
            if (relative) {
                const T * Pr = Ph + (size_t) row * p_sr + lane * 4;
                const float4 p4 = attention_load4(Pr);
                s += p4.x * Qv[lane * 4 + 0] + p4.y * Qv[lane * 4 + 1] +
                     p4.z * Qv[lane * 4 + 2] + p4.w * Qv[lane * 4 + 3];
            }
#pragma unroll
        ...
        // Preserve one thread per key (important for Magpie's long cache) while reducing each
        // dot product from 64 scalar loads/instructions to 16 aligned float4 operations.
        for (int j = key_begin + d; j < kv; j += dk) {
            const int row = (q - 1) + j - i;
            float ac = 0.0f;
            float bd = 0.0f;
#pragma unroll
            for (int dd = 0; dd < 64; dd += 4) {
                ...
                const float4 qu4 = attention_load4(Qu + dd);
                ac += k4.x * qu4.x + k4.y * qu4.y + k4.z * qu4.z + k4.w * qu4.w;
                if (relative) {
                    const float4 p4 = attention_load4(Ph + (size_t) row * p_sr + dd);
                    const float4 qv4 = attention_load4(Qv + dd);
                    bd += p4.x * qv4.x + p4.y * qv4.y + p4.z * qv4.z + p4.w * qv4.w;
                }
            }
    ...
    float lm = -INFINITY;
    for (int j = key_begin + d; j < kv; j += dk) lm = fmaxf(lm, sc[j]);
    red[d] = lm;
    __syncthreads();
    ...
    float c = 0.0f;
    for (int j = key_begin; j < kv; j++) {
        c += sc[j] * attention_load_kv<T, Cached>(
                         Vh, Vch, j, cache_len, ring_head, v_sj, cache_sj, d);
    }
    ctx[(size_t) b * o_sb + (size_t) i * o_si + (size_t) h * o_sh + d] = c * inv;
}

void ggml_cuda_op_fused_attention(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const ggml_tensor * cache_state = dst->src[9];
    const bool cached = kv_cache != nullptr;
    const bool relative = p != nullptr;

    GGML_ASSERT(q->type == GGML_TYPE_F32);
    // K/V/P may be F32 or (all together) F16 — see the kernel comment.
    GGML_ASSERT(k->type == v->type);
    GGML_ASSERT(!relative || k->type == p->type);
    GGML_ASSERT(k->type == GGML_TYPE_F32 || k->type == GGML_TYPE_F16);
    GGML_ASSERT(!relative || (bias_u->type == GGML_TYPE_F32 && bias_v->type == GGML_TYPE_F32));
    ...
    GGML_ASSERT((d_k & (d_k - 1)) == 0 && "fused_attention: d_k must be a power of two");
    ...
    if (cached) {
        ...
        GGML_ASSERT(cache_state->ne[0] == batch);
    ...
    const long p_sr = relative ? (long)(p->nb[1] / ke) : 0;
    const long p_sh = relative ? (long)(p->nb[2] / ke) : 0;
    ...
    GGML_ASSERT(shmem <= max_shmem && "fused_attention: kv window too large for shared memory");
    ...
    const int32_t * active_ring_heads =
        cached ? (const int32_t *) cache_state->data : nullptr;
    const int32_t * active_lengths =
        cached && cache_state->ne[1] == 2
            ? active_ring_heads + cache_state->ne[0]
            : nullptr;
    ...
    auto update_cache = [&]() {
        ...
        if (k->type == GGML_TYPE_F16) {
            fused_attention_update_cache_kernel<half><<<update_grid, 256, 0, stream>>>(
            ...
            fused_attention_update_cache_kernel<float><<<update_grid, 256, 0, stream>>>(
    ...
    const bool use_cached_q1_warp64 = !relative && cached &&
        GGML_CUDA_CC_IS_NVIDIA(device_info.cc) && device_info.warp_size == 32 &&
        d_k == 64 && q_len == 1;
    if (use_cached_q1_warp64) {
        ...
        if (k->type == GGML_TYPE_F16) {
            fused_cached_attention_q1_d64_kernel<half, true>
                <<<specialized_grid, threads, specialized_shmem, stream>>>(
            ...
            fused_cached_attention_q1_d64_kernel<float, true>
                <<<specialized_grid, threads, specialized_shmem, stream>>>(
    ...
    // Select fixed-shape relative-position kernels when their geometry and
    // device requirements are met. For Q=2, KV=72, occupancy determines
    // whether one block processes one or two query rows.
    const bool use_sm100_q2 = relative &&
        device_info.cc == RELPOS_ATTN_CC_SM100 && device_info.warp_size == 32 &&
        d_k == RELPOS_ATTN_DK_128 && q_len == 2 && kv_len == 72 && m_sq == 0;
    const bool use_register_q2 = relative &&
    ...
    const bool use_register_q4 = relative &&
        GGML_CUDA_CC_IS_NVIDIA(device_info.cc) && device_info.cc >= GGML_CUDA_CC_AMPERE &&
    ...
    const bool use_register_common = relative &&
        GGML_CUDA_CC_IS_NVIDIA(device_info.cc) && device_info.cc >= GGML_CUDA_CC_AMPERE &&
    ...
    auto launch_generic = [&](auto scalar, auto cache_tag) {
        using T = decltype(scalar);
        constexpr bool Cached = decltype(cache_tag)::value;
        fused_attention_kernel<T, Cached><<<grid, d_k, shmem, stream>>>(
            (const float *) q->data, (const T *) k->data, (const T *) v->data,
            relative ? (const T *) p->data : nullptr,
            relative ? (const float *) bias_u->data : nullptr,
            relative ? (const float *) bias_v->data : nullptr,
            mask ? (const float *) mask->data : nullptr, (float *) dst->data,
            cache_k, cache_v, active_slots, active_ring_heads, active_lengths, q_len, kv_len,
            n_head, cache_len, scale, relative,
