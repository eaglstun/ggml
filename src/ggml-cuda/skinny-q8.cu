...
template <int n> static __device__ __forceinline__ void skq8_wait() {
...
std::unordered_map<const void *, skq8_planes> g_skq8_cache;
...
static void skq8_run(
    ...
    skq8_planes planes = {};
    {
        ...
        if (it != g_skq8_cache.end()) {
            ...
            if ((src0->flags & GGML_TENSOR_FLAG_Q8_PLANAR) != 0) {
                ...
                size_t        extra  = 0;
                if (inplace) {
                    ggml_cuda_pool_alloc<int8_t> staging(ctx.pool(), qs_bytes + d_bytes);
                    skq8_repack<<<blocks, 256, 0, stream>>>(
        ...
        if (use_cublas_f16 && planes.f16 == nullptr) {
            ...
            skq8_dequantize_weight_f16<<<blocks, 256, 0, stream>>>(
    ...
    if (use_cublas_f16) {
        ...
        if (src1->type == GGML_TYPE_F16) {
            ...
            ggml_cuda_pool_alloc<half> a_f16(ctx.pool(), (size_t) N * K);
    ...
    ggml_cuda_pool_alloc<int8_t> aq_alloc(ctx.pool(), (size_t) ntot * K);
    ggml_cuda_pool_alloc<float>  ad_alloc(ctx.pool(), (size_t) ntot * KB);
    {
        const int64_t warps  = (int64_t) ntot * KB;  // one warp per (col, q8 block)
        const int     blocks = (int) ((warps * 32 + 255) / 256);
        skq8_quantize<<<blocks, 256, 0, stream>>>(
    ...
    // tile, but all tiles are visible to the scheduler in a single launch.
    {
        ...
        if (ksplit > 1) {
            const int64_t count = M * N;
            ggml_cuda_pool_alloc<float> partials(ctx.pool(), (size_t) ksplit * count);
            skq8_gemm<<<grid, SKQ8_WARPS * 32, smem, stream>>>(
